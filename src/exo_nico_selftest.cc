/*  =========================================================================
    exo_nico_selftest.c - run selftests
...
// Tests for draft public classes:
    { "exo_nico_server", exo_nico_server_test },
    { "exo_nico_client", exo_nico_client_test },
#endif // EXO_NICO_BUILD_DRAFT_API
#ifdef EXO_NICO_BUILD_DRAFT_API
    { "private_classes", exo_nico_private_selftest },
#endif // EXO_NICO_BUILD_DRAFT_API
    {0, 0}          //  Sentinel
};
...
{
    test_item_t *item;
    printf ("Running exo-nico selftests...\n");
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("exo_nico_selftest.c [options] ...");
        ...
        ||  streq (argv [argn], "-l")) {
            ...
            puts ("    exo_nico_server\t\t- draft");
            puts ("    exo_nico_client\t\t- draft");
    ...
    if (test) {
        printf ("Running exo-nico test '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
